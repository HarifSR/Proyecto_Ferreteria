...
import { alertaExito, alertaError, alertaAdvertencia, confirmarAccion } from './utils/alerts';
import { formatQ } from './utils/format';
import { exportarReportesExcel } from './utils/exportExcel';
...
export default function App() {
  ...
  return (
    ...
    <div className="min-h-screen bg-gray-100 font-sans text-gray-800">
      {!token ? (
        /* Pantalla de Login */
        <LoginScreen email={email} setEmail={setEmail} password={password} setPassword={setPassword} manejarLogin={manejarLogin} />
      ) : (
        <div className="flex h-screen relative">
          <Sidebar
            subSeccionAdmin={subSeccionAdmin} setSubSeccionAdmin={setSubSeccionAdmin}
            modoCatalogo={modoCatalogo} setModoCatalogo={setModoCatalogo}
            usuarioActual={usuarioActual} cerrarSesion={cerrarSesion}
            menuMovilAbierto={menuMovilAbierto} setMenuMovilAbierto={setMenuMovilAbierto}
            limpiarFormulario={limpiarFormulario} limpiarFormularioUsuario={limpiarFormularioUsuario}
          />
            ...
            {subSeccionAdmin === 'nuevo-producto' && (
              <div className="space-y-6">
                ...
                <div className="bg-white p-6 rounded-xl border shadow-sm space-y-4">
                  ...
                  <p className="text-[11px] text-gray-400 text-center">Revisa el historial completo en la pestaña "Historial".</p>
                </div>
                  ...
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y">
                      {productosFiltradosAdmin.length === 0 ? (
                        ...
                        <tr key={p.id} onClick={() => setProductoDetalle(p)} className="hover:bg-gray-50 transition cursor-pointer">
    ...
    <ProductoDetalleModal productoDetalle={productoDetalle} setProductoDetalle={setProductoDetalle} />
    </>
  );
}